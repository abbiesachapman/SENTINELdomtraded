...
## Abbie Chapman, last edited Feb 2026
## TOR calculations on updated SPAM data and crops identified in 0_Trade_Matrices
## This script was repeated for birds and mammals only and for threatened species. These separate scripts are available on request, as 
## they essentially repeat this script but with birds and mammals only as the species data or a threatened species subset.

## Read in output of 0_Trade_Data_Read_In.R first
## Note that I moved the output files to a different destination as sending to the SENTINEL 2017 data folder didn't
## make sense as then it wouldn't read in properly when re-run. Instead, these files are in Data>Overlay Analysis> August 2023>Data_Outputs
...
outdir
cropdata.dir = "MapSPAM/2020_version/spam2020V1r0_global_physical_area/" # using physical area data
...
## Cropping to the SENTINEL project focal countries - this is slow to run so is commented out:
#########
...
average_expectation_GHA = (median_SR*(median_area/median_cell_area))/(median_area)
...
newdir = "C:/Users/Dr Abbie/Documents/Manuscripts/Overlay Analysis/Revision/"
...
average_expectation_TRP_GHA = (median_SR*(median_area/median_cell_area))
...
# Now, I want to use these values to get the TOR average expectation:
average_expectation_ETH = (median_SR*(median_area/median_cell_area))/(median_area)
...
# for relative risk potential, I could take the risk potential again and then divide by the average expectation
# but with the average expectation calculated from just the top part of the equation too
average_expectation_TRP_ETH = (median_SR*(median_area/median_cell_area))
...
# Now, I want to use these values to get the TOR average expectation:
average_expectation_ZMB = (median_SR*(median_area/median_cell_area))/(median_area)
...
# for relative risk potential, I could take the risk potential again and then divide by the average expectation
# but with the average expectation calculated from just the top part of the equation too
average_expectation_TRP_ZMB = (median_SR*(median_area/median_cell_area))
# can add this to the risk potential calculated further down later
...
predom_traded = filter(alldata, proportion_traded >= 60) # we only get 4 crops - coffee, banana, cocoa (Ghana) and sesame (Ethiopia)
predom_traded = filter(alldata, proportion_traded >= 50) # we still only get one crop for Zambia
predom_traded = filter(alldata, proportion_traded > 50) # we still only get one crop for Zambia
predom_traded = filter(alldata, proportion_traded >= 20)
# Instead of a threshold, maybe it makes more sense to say if more is exported than kept?
predom_traded = filter(alldata, total_export_tonnes_all_reporters > domestic_tonnes)
# still not many
View(alldata)
...
# quick checks on things for manuscript info:
...
# write.csv(alldata, paste0(outdir, "alldata.csv"))


alldata$label = paste(alldata$crop, ",", alldata$country)

(plot1_allthreecountries = ggplot(alldata, aes(x = domtraded, y = tor, fill = domtraded)) +
    geom_boxplot(fill = c("#009E73", "#F0E442")) +
    geom_point(size = 2, alpha = 0.9) +
    labs(x = "Crop Group", y = "Trade-Off Risk", fill = "Crop") +
    theme_classic() +
    # geom_text(data=subset(alldata, tor > 6 | tor < 2), aes(domtraded,tor,label=label),
...
(plot1_ghana = ghanadata %>% 
    ggplot(aes(x=domtraded,y=tor, label = crop))+
    geom_boxplot(width=.5, colour = c("#FF6F00FF", "#8A4198FF"))+
    ...
    theme(legend.position="none",
          text = element_text(family="sans", size = 12), # sans serif
          axis.title.y = element_text(size = 12, vjust = 5, face = "bold"), # ensuring position of axis label is away from the ticks
          axis.title.x = element_text(size = 12, vjust = 5, face = "bold"),
          axis.text.y = element_text(size = 12),
          axis.ticks.length.x = unit(-0.1, "cm"), # length of tick marks - negative sign places ticks inwards
          axis.ticks = element_blank(),
          axis.text.x = element_text(size = 12),
...
(plot1_allthreecountries = ggplot(alldata_risk, aes(x = domtraded, y = tor, fill = domtraded)) +
    geom_boxplot(fill = c("#009E73", "#F0E442")) +
    geom_point(size = 2, alpha = 0.9) +
    labs(x = "Crop Group", y = "Trade-Off Risk", fill = "Crop") +
    theme_classic() +
    # geom_text(data=subset(alldata_risk, tor > 6 | tor < 2), aes(domtraded,tor,label=label),
    #                                                        position = position_jitter(width = -0.1, height = 0.2)) +
    theme(text = element_text(family="sans"), # sans serif
          axis.title.y = element_text(size = 12, vjust = 5, face = "bold"), # ensuring position of axis label is away from the ticks
          axis.title.x = element_text(size = 12, vjust = 1, face = "bold"),
...
(plot1_all = alldata_risk %>% 
    ...
    # jittered text with geom_text
    theme_bw()+
    labs(x = "Crop Group", y = "Total Risk Potential") +
    theme(legend.position="none",
          text = element_text(family="sans"), # sans serif
          axis.title.y = element_text(size = 12, vjust = 2, face = "bold"), # ensuring position of axis label is away from the ticks
          axis.title.x = element_text(size = 12, vjust = -2, face = "bold"),
